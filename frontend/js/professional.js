...
const state = {
  professional: JSON.parse(sessionStorage.getItem("professionalSession") || "null"),
  appointments: [],
...
document.getElementById("login-form")?.addEventListener("submit", async (event) => {
  ...
  try {
    const data = await loginProfessional(
    ...
    sessionStorage.setItem("professionalSession", JSON.stringify(state.professional));
    setView();
  } catch (err) {
    error.hidden = false;
  }
...
async function loadAppointments() {
  ...
  try {
    state.appointments = await fetchProfessionalAppointments(state.professional.id);
    renderAppointments();
  } catch (err) {
...
function renderAppointments() {
  ...
  document.getElementById("today-label").textContent = today.toLocaleDateString("es-CO", {
    weekday: "long",
    day: "numeric",
    month: "short",
  });
  ...
  if (!appointments.length) {
    ...
    document.getElementById("reminder-text").textContent = "No tienes citas pendientes por revisar.";
    return;
  }
  ...
  listEl.innerHTML = appointments.map((appt) => {
    ...
    const image = appt.design_image_url || appt.service.image_url || "https://images.unsplash.com/photo-1604654894610-df63bc536371?w=240&q=80";
    return `
  ...
  listEl.querySelectorAll(".pro-appointment-card").forEach((card) => {
    card.addEventListener("click", () => openAppointment(Number(card.dataset.id)));
  });
}
...
function openAppointment(id) {
  ...
  const date = new Date(appt.appointment_datetime);
  const image = appt.design_image_url || appt.service.image_url || "https://images.unsplash.com/photo-1604654894610-df63bc536371?w=720&q=85";
...
document.getElementById("modal-close")?.addEventListener("click", () => closeModal(appointmentModal));
document.getElementById("cancel-appointment-btn")?.addEventListener("click", () => cancelModal.classList.add("open"));
document.getElementById("complete-appointment-btn")?.addEventListener("click", () => completeModal.classList.add("open"));
document.getElementById("confirm-appointment-btn")?.addEventListener("click", () => confirmModal.classList.add("open"));
document.getElementById("cancel-back-btn")?.addEventListener("click", () => closeModal(cancelModal));
document.getElementById("complete-back-btn")?.addEventListener("click", () => closeModal(completeModal));
document.getElementById("confirm-back-btn")?.addEventListener("click", () => closeModal(confirmModal));
...
document.getElementById("complete-confirm-btn")?.addEventListener("click", async () => {
  if (!state.selectedAppointment) return;
...
document.getElementById("confirm-confirm-btn")?.addEventListener("click", async () => {
  if (!state.selectedAppointment) return;
...
function renderMiniWeek() {
  ...
  week.innerHTML = labels.map((label, index) => {
    const date = new Date(start);
    date.setDate(start.getDate() + index);
    const active = date.toDateString() === state.selectedBlockDate.toDateString();
    return `
...
function renderQuickSlots() {
  document.querySelectorAll("#quick-slots button").forEach((btn) => {
    ...
    btn.addEventListener("click", () => {
      state.selectedBlockSlot = { start: btn.dataset.start, end: btn.dataset.end };
      renderQuickSlots();
    });
  });
}
